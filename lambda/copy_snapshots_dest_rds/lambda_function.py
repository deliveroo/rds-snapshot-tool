...
def lambda_handler(event, context):
    ...
    response = paginate_api_call(
        client, "describe_db_snapshots", "DBSnapshots", IncludeShared=True
    )
    ...
    response_dest = paginate_api_call(
        client_dest, "describe_db_snapshots", "DBSnapshots"
    )
    ...
    for shared_identifier, shared_attributes in shared_snapshots.items():
        ...
        if (
            shared_identifier not in own_snapshots.keys()
            and shared_identifier not in own_dest_snapshots.keys()
        ):
            # Check date
            ...
            if creation_date:
                ...
                if days_difference < RETENTION_DAYS:
                    ...
                    logger.info(
                        "Not copying %s locally. Older than %s days"
                        % (shared_identifier, RETENTION_DAYS)
                    )

            else:
                logger.info(
                    "Not copying %s locally. No valid timestamp" % shared_identifier
                )

        # Copy to DESTINATION_REGION
        elif (
            shared_identifier not in own_dest_snapshots.keys()
            and shared_identifier in own_snapshots.keys()
        ...
        # Delete local snapshots
            ...
            shared_identifier in own_dest_snapshots.keys()
            and shared_identifier in own_snapshots.keys()
            and own_dest_snapshots[shared_identifier]["Status"] == "available"
            and REGION != DESTINATION_REGION
        ):

            response = client.delete_db_snapshot(DBSnapshotIdentifier=shared_identifier)
