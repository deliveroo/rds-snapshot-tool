...
def get_own_snapshots_no_x_account(pattern, response, REGION):
    ...
    for snapshot in response["DBSnapshots"]:
        ...
        if (
            ...
            and re.search(pattern, snapshot["DBSnapshotIdentifier"])
...
def get_shared_snapshots(pattern, response):
    ...
    for snapshot in response["DBSnapshots"]:
        if (
            ...
            and re.search(pattern, get_snapshot_identifier(snapshot))
            and snapshot["Engine"] in _SUPPORTED_ENGINES
        ):
...
class DBSnapshot:
    def __init__(self, db_snapshot_dict, filtered_dict):
        self.db_snapshot_dict = db_snapshot_dict
        self.filtered_dict = filtered_dict

    def __getitem__(self, key):
        return self.filtered_dict.__getitem__(key)

    @property
    def is_own_snapshot(self):
        client = boto3.client("rds", region_name=_REGION)
        response_tags = client.list_tags_for_resource(
            ResourceName=self.db_snapshot_dict["DBSnapshotArn"]
        )

        return search_tag_created(response_tags)
...
def get_own_snapshots_source(pattern, response):
    ...
    for snapshot in response["DBSnapshots"]:
        if (
            ...
            filtered[snapshot["DBSnapshotIdentifier"]] = DBSnapshot(
                snapshot,
                {
                    "Arn": snapshot["DBSnapshotArn"],
                    "Status": snapshot["Status"],
...
def paginate_api_call(client, api_call, objecttype, *args, **kwargs):
    ...
    while "Marker" in temp_response:
        if kwargs:
            ...
            temp_response = eval(
                'client.%s(Marker="%s")' % (api_call, temp_response["Marker"])
            )
